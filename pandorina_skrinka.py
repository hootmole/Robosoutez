...
class Robot:
    ...
    def drive_until_obstacle(self, distance_to_object, kp=1.5, correction_distance=50):
        ...
        print("dist: " + str(current_distance) + " mm")
        
        # Calculate the distance to travel and the corresponding wheel rotation
        target_distance = current_distance - distance_to_object - correction_distance  # Distance to close the gap + distance for adjustment
        target_angle = (target_distance * 360) / (math.pi * self.wheel_diameter)
        ...
        while True:
            ...
            left_position = self.left_motor.angle()
            right_position = self.right_motor.angle()
            avg_position = (left_position + right_position) / 2

            # Break if the target distance is reached
            print(self.us_sensor.distance(), avg_position, target_distance, int(avg_position) > int(target_angle))
            if int(avg_position) > int(target_angle):
                break
